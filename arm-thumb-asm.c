...
static int thumb_emit_opcode(thumb_opcode op) {
  if (op.size == 0) {
    return op.size;
  }
  if (op.size == 4) {
...
static void asm_emit_opcode_2(int token, uint32_t opcode, const char *file,
                              int line) {
  tcc_error("Reimplement me to Thumb: token: %x, opcode: %x from: %s:%d\n",
            token, opcode, file, line);
  gen_le32((condition_code_of_token(token) << 28) | opcode);
}

#define asm_emit_opcode(token, opcode)                                         \
  asm_emit_opcode_2(token, opcode, __FILE__, __LINE__)

static void thumb_emit_t16_opcode(uint16_t opcode) { gen_le16(opcode); }

static void thumb_emit_t32_opcode(uint32_t opcode) {
  gen_le16(opcode >> 16);
  gen_le16(opcode & 0xffff);
}
...
static void asm_unary_opcode(TCCState *s1, int token) {
  ...
  case TOK_ASM_svceq:
    if (op.type != OP_IM8)
      ...
      thumb_emit_t16_opcode(0xdf00 | op.e.v);
    }
    break;
  default:
    ...
    case TOK_ASM_movweq:
      if (ops[1].type == OP_IM8 || ops[1].type == OP_IM8N ||
        ...
        if (ops[1].e.v >= 0 && ops[1].e.v <= 0xFFFF) {
          ...
          switch (ARM_INSTRUCTION_GROUP(token)) {
          case TOK_ASM_movteq:
            thumb_emit_opcode(th_mov_imm(ops[0].reg, immediate_value));
            break;
          case TOK_ASM_movweq:
...
static void asm_data_processing_opcode(TCCState *s1, int token) {
  ...
  if (nb_ops != 3) {
    ...
    thumb_emit_t32_opcode(opcode | operands);
  }
}
...
static void asm_branch_opcode(TCCState *s1, int token) {
  ...
  case TOK_ASM_bxeq:
    if (op.type != OP_REG32)
      ...
      thumb_emit_t16_opcode(0x4700 | (op.reg << 3));
    break;
  case TOK_ASM_blxeq:
    if (op.type != OP_REG32)
      expect("register");
