...
static int irop_btype_to_vt_btype_for_load(int irop_btype, int is_unsigned);
...
static void thumb_backend_dump_svalue(const char *label, const SValue *sv)
{
  ...
  fprintf(stderr, "%s: r=0x%x pr0=%d pr1=%d vr=%d c.i=%lld type=0x%x flags=0x%x sym=%p\n", label, sv->r, pr0_packed,
          pr1_packed, sv->vr, (long long)sv->c.i, sv->type.t, sv->type.ref ? sv->type.ref->type.t : 0, (void *)sv->sym);
}
...
static void load_to_reg(int r, int r1, SValue *src);
...
static ScratchRegAllocs get_scratch_regs_with_save(uint32_t exclude_regs, int count)
{
  ...
  for (int i = 0; i < count; ++i)
  {
    ...
    if (reg != PREG_NONE && reg >= 0 && reg < 16)
    {
      ...
      result.regs[i] = reg;
      exclude |= (1u << reg);
      /* Mark as globally excluded while live so nested helpers cannot
       * accidentally reuse it and clobber a still-needed operand. */
                                      ...
                                      ScratchRegAlloc *base_alloc, int *has_base_alloc)
{
  ...
  *has_base_alloc = 0;
  ...
  if ((fr & VT_LVAL) && v < VT_CONST)
  {
     ...
     */
    base_reg = (sv->pr0_reg != PREG_REG_NONE) ? sv->pr0_reg : v;
...
/* IROperand version of th_store_resolve_base */
...
static void store_ir(int r, IROperand sv)
{
  store_ex_ir(r, sv, 0);
}
...
/* Convert IROP_BTYPE to VT_BTYPE for load operations */
static int irop_btype_to_vt_btype_for_load(int irop_btype, int is_unsigned)
{
  int vt;
  switch (irop_btype)
 ...
 * This is a simplified version of load_vt_lval_vt_local that doesn't need SValue.
 */
static void load_vt_lval_vt_local_from_base(int r, int r1, int ft, int fc, int sign, uint32_t base)
{
  int success = 0;
  const int btype = ft & VT_BTYPE;

  TRACE("load_vt_lval_vt_local_from_base: r=%d, r1=%d, ft=0x%x, fc=%d, sign=%d, base=%d", r, r1, ft, fc, sign, base);
  ...
  if (is_float(ft))
  {
    ...
    if (btype == VT_DOUBLE || btype == VT_LDOUBLE)
    {
      /* Double: load 64 bits to register pair */
      int ir_high = r1;
    ...
    /* Single float: load as word */
    success = load_word_from_base(r, base, fc, sign);
  }
  else if (btype == VT_LLONG)
  {
    /* 64-bit integer - load to register pair */
    int ir_high = r1;
    if (ir_high < 0 || ir_high == PREG_REG_NONE)
    {
      ir_high = r + 1;
    ...
    fprintf(stderr, "DEBUG load_vt_lval_vt_local_from_base VT_LLONG: r=%d ir_high=%d base=%d fc=%d sign=%d\n", r,
            ir_high, (int)base, fc, sign);
    ...
    if (base_reg == (uint32_t)r || base_reg == (uint32_t)ir_high)
    {
      fprintf(stderr, "DEBUG load_vt_lval_vt_local_from_base: BASE OVERLAPS! allocating scratch\n");
      uint32_t exclude = (1u << r) | (1u << ir_high);
      base_alloc = get_scratch_reg_with_save(exclude);
      base_reg = (uint32_t)base_alloc.reg;
      fprintf(stderr, "DEBUG load_vt_lval_vt_local_from_base: scratch_reg=%d\n", (int)base_reg);
      ot_check(th_mov_reg((int)base_reg, (int)base, FLAGS_BEHAVIOUR_NOT_IMPORTANT, THUMB_SHIFT_DEFAULT,
                          ENFORCE_ENCODING_NONE, false));
    }
...
{
  fprintf(stderr,
          "DEBUG load_to_dest: src->vr=%d src->r=0x%x VT_PARAM=%d VT_LOCAL=%d VT_LVAL=%d src->c.i=%d dest->pr0_reg=%d "
          "dest->pr1_reg=%d\n",
          src->vr, src->r, (src->r & VT_PARAM) ? 1 : 0, (src->r & 0xff) == VT_LOCAL ? 1 : 0, (src->r & VT_LVAL) ? 1 : 0,
          (int)src->c.i, dest->pr0_reg, dest->pr1_reg);
  IROperand d = svalue_to_iroperand(tcc_state->ir, dest);
  IROperand s = svalue_to_iroperand(tcc_state->ir, src);
  fprintf(stderr,
          "DEBUG load_to_dest after conversion: s.vr=%d s.tag=%d s.is_param=%d s.is_lval=%d s.u.imm32=%d d.pr0_reg=%d "
          "d.pr1_reg=%d\n",
          s.vr, s.tag, s.is_param, s.is_lval, s.u.imm32, d.pr0_reg, d.pr1_reg);
  load_to_dest_ir(d, s);
}
...
void load_to_dest_ir(IROperand dest, IROperand src)
{
  ...
  fprintf(stderr, "DEBUG load_to_dest_ir ENTRY: tag=%d is_param=%d\n", tag, src.is_param);
  ...
  case IROP_TAG_VREG:
  {
    ...
    if (src.is_lval)
    {
      ...
      int ft = irop_btype_to_vt_btype_for_load(btype, src.is_unsigned);
      load_vt_lval_vt_local_from_base(dest.pr0_reg, pr1_for_load, ft, 0, 0, src_reg);
      return;
    }
  ...
  case IROP_TAG_STACKOFF:
  {
    ...
    if (src.is_param)
    {
      fprintf(
          stderr,
          "DEBUG load_to_dest_ir STACKOFF: frame_offset=%d is_param=%d offset_to_args=%d is_lval=%d dest.pr0_reg=%d\n",
          frame_offset, src.is_param, offset_to_args, src.is_lval, dest.pr0_reg);
    }
#endif
    ...
    if (src.is_lval)
    {
      ...
      int pr1_for_load = dest.pr1_spilled ? PREG_REG_NONE : dest.pr1_reg;
      int ft = irop_btype_to_vt_btype_for_load(btype, src.is_unsigned);
      load_vt_lval_vt_local_from_base(dest.pr0_reg, pr1_for_load, ft, abs_offset, sign, base_reg);
    }
    else
    {
  ...
  case IROP_TAG_F64:
  {
    ...
    tcc_machine_load_constant(dest.pr0_reg, dest.pr1_spilled ? PREG_REG_NONE : dest.pr1_reg, (int64_t)value, 1, NULL);
    return;
  }
  case IROP_TAG_SYMREF:
  {
    ...
    if (src.is_lval)
    {
      ...
      load_vt_lval_vt_local_from_base(dest.pr0_reg, pr1_for_const, ft, abs_offset, sign, base_reg);
...
static void load_to_reg(int r, int r1, SValue *src)
{
  fprintf(stderr, "DEBUG load_to_reg: r=%d r1=%d src->r=0x%x VT_PARAM=%d VT_LOCAL=%d\n", r, r1, src->r,
          (src->r & VT_PARAM) ? 1 : 0, (src->r & 0xff) == VT_LOCAL ? 1 : 0);
  IROperand s = svalue_to_iroperand(tcc_state->ir, src);
  fprintf(stderr, "DEBUG load_to_reg after conversion: s.tag=%d s.is_param=%d\n", s.tag, s.is_param);
...
/* Legacy load() - loads SValue to a register using the old code path.
 * This is kept for backward compatibility with existing code that uses SValue.
 * New code should use load_to_dest_ir() with IROperand. */
void load(int r, SValue *sv)
{
  ...
  /* Apply offset_to_args for stack parameters */
  ...
  if (fr & VT_LVAL)
  {
    ...
    else if (v < VT_CONST)
    {
      ...
      {
        thumb_require_materialized_reg(ctx, "lvalue base", sv->pr0_reg);
        base = sv->pr0_reg;
      }
      else
      {
    ...
    if (v == VT_LOCAL)
    {
      int pr1_for_load = sv->pr1_spilled ? PREG_REG_NONE : sv->pr1_reg;
      return load_vt_lval_vt_local(r, pr1_for_load, sv, ft, fc, sign, base);
    }
  }
  else if (v == VT_CONST)
  {
    Sym *sym = (sv->r & VT_SYM) ? sv->sym : NULL;
    int is_64bit = tcc_ir_is_64bit(ft);
    int pr1_for_const = sv->pr1_spilled ? PREG_REG_NONE : sv->pr1_reg;
    return tcc_machine_load_constant(r, pr1_for_const, sv->c.i, is_64bit, sym);
  }
  else if (v == VT_LOCAL)
  {
    int base = tcc_state->need_frame_pointer ? R_FP : R_SP;
    return load_vt_local(r, sv, base);
  }
  else if (v == VT_CMP)
    return tcc_machine_load_cmp_result(r, sv->c.i);
  else if (v == VT_JMP || v == VT_JMPI)
    return tcc_machine_load_jmp_result(r, sv->c.i, v == VT_JMPI);
  else if (v < VT_CONST)
  {
    int src_reg = v;
    if (sv->pr0_reg != PREG_REG_NONE)
    {
      thumb_require_materialized_reg(ctx, "source register", sv->pr0_reg);
      src_reg = sv->pr0_reg;
    }
  ...
  tcc_error("compiler_error: unknown load in legacy load(): v=%d, fr=0x%x, ft=0x%x\n", v, fr, ft);
}
                                                 ...
                                                 bool *store_low, bool *store_high, uint32_t *exclude_mask)
{
  ...
  *store_high = false;
  ...
  /* Defensive fix: if a 64-bit register destination is missing the high register,
   * assume a consecutive pair (r, r+1). This prevents storing the high word into
   * the low register when pr1 is unset.
   */
  if (((*rd_high == PREG_REG_NONE) || (*rd_high == *rd_low)) && dest->pr0_reg != PREG_REG_NONE &&
      !(dest->r & VT_LVAL) && dest->r != VT_LOCAL)
  {
...
static void thumb_store_dest_pair_if_needed(SValue *dest, int rd_low, int rd_high, bool store_low, bool store_high)
{
  ...
  const bool dest_is_reg =
      (!(dest->r & VT_LVAL) && dest->r != VT_LOCAL && dest->pr0_reg != PREG_REG_NONE && thumb_is_hw_reg(dest->pr0_reg));
  ...
  if (store_low)
  {
    ...
    else
    {
      ...
      dest_lo.type.t = VT_INT;
      store(rd_low, &dest_lo);
    }
  }
  if (store_high)
  {
...
static void thumb_emit_opcode64_imm(SValue *src1, SValue *src2, SValue *dest, TccIrOp op, const char *ctx,
                                    ThumbDataProcessingHandler regular, ThumbDataProcessingHandler carry)
{
  const bool src2_is_imm = th_has_immediate_value(src2->r);
  const uint64_t src2_imm = (uint64_t)src2->c.i;
  ...
  thumb_prepare_dest_pair_for_64bit_op(ctx, dest, &rd_low, &rd_high, &rd_low_alloc, &rd_high_alloc, &store_low,
                                       &store_high, &exclude);

  const bool src1_is64 = is_64bit_type(src1->type.t);
  const bool src2_is64 = is_64bit_type(src2->type.t);

  /* Materialize src1. */
  const bool src1_is_imm = (src1->pr0_reg == PREG_REG_NONE) && th_has_immediate_value(src1->r);
  ...
  const bool src1_needs_value_load = src1_is_address_of || src1_is_sym_address;
  ...
  if (src1_is_imm)
  {
    ...
    if (src1_is64)
    {
      tcc_machine_load_constant(rd_low, rd_high, src1->c.i, 1, sym);
      rn_low = rd_low;
      rn_high = rd_high;
    }
    else
    {
      tcc_machine_load_constant(rd_low, PREG_NONE, src1->c.i, 0, sym);
      rn_low = rd_low;
      rn_high = PREG_REG_NONE;
    }
  }
  else if (!src1_needs_value_load && !(src1->r & VT_LVAL) && thumb_is_hw_reg(rn_low) &&
           (!src1_is64 || (rn_high != PREG_REG_NONE && thumb_is_hw_reg(rn_high))))
  {
    thumb_require_materialized_reg(ctx, "src1.low", rn_low);
    if (src1_is64 && rn_high != PREG_REG_NONE)
  ...
  else
  {
    ...
    if (src1_is64)
    {
      ...
      exclude |= (1u << rn_high);
      SValue src1_tmp = *src1;
      if (src1_needs_value_load)
        src1_tmp.r |= VT_LVAL;
      load_to_reg(rn_low, rn_high, &src1_tmp);
    }
    else
    {
      rn_high = PREG_REG_NONE;
      ...
      if (src1_needs_value_load)
        src1_tmp.r |= VT_LVAL;
      load_to_reg(rn_low, PREG_NONE, &src1_tmp);
    }
  }
  ...
  const int src2_kind = src2->r & VT_VALMASK;
  const bool src2_is_address_of = (src2_kind == VT_LOCAL || src2_kind == VT_LLOCAL) && !(src2->r & VT_LVAL);
  const bool src2_is_sym_address = (src2->r & VT_SYM) && !(src2->r & VT_LVAL);
  const bool src2_needs_value_load = src2_is_address_of || src2_is_sym_address;
  if (!src2_is_imm)
  {
    if (!src2_needs_value_load && !(src2->r & VT_LVAL) && thumb_is_hw_reg(rm_low) &&
        (!src2_is64 || (rm_high != PREG_REG_NONE && thumb_is_hw_reg(rm_high))))
    {
      thumb_require_materialized_reg(ctx, "src2.low", rm_low);
      if (src2_is64 && rm_high != PREG_REG_NONE)
    ...
    else
    {
      ...
      if (src2_is64)
      {
        ...
        exclude |= (1u << rm_high);
        SValue src2_tmp = *src2;
        if (src2_needs_value_load)
          src2_tmp.r |= VT_LVAL;
        load_to_reg(rm_low, rm_high, &src2_tmp);
      }
      else
      {
        ...
        if (src2_needs_value_load)
          src2_tmp.r |= VT_LVAL;
        load_to_reg(rm_low, PREG_NONE, &src2_tmp);
      }
    }
  }
  else
  ...
  thumb_store_dest_pair_if_needed(dest, rd_low, rd_high, store_low, store_high);
...
static void thumb_emit_logical64_op(SValue *src1, SValue *src2, SValue *dest, TccIrOp op,
                                    ...
                                    thumb_u32_fold_t fold32, const char *ctx)
{
  ...
  /* Only treat true VT_CONST operands as immediates.
   ...
   */
  const bool src1_is_imm = th_has_immediate_value(src1->r);
  const bool src2_is_imm = th_has_immediate_value(src2->r);
  const uint64_t src1_imm = (uint64_t)src1->c.i;
  const uint64_t src2_imm = (uint64_t)src2->c.i;

  /* If the destination vreg is the same as a source, but pr1 is missing or
   * corrupted, inherit the source high register to avoid clobbering the low. */
  uint8_t dest_pr0_packed = (dest->pr0_spilled ? PREG_SPILLED : 0) | dest->pr0_reg;
  uint8_t dest_pr1_packed = (dest->pr1_spilled ? PREG_SPILLED : 0) | dest->pr1_reg;
  uint8_t src1_pr0_packed = (src1->pr0_spilled ? PREG_SPILLED : 0) | src1->pr0_reg;
  uint8_t src2_pr0_packed = (src2->pr0_spilled ? PREG_SPILLED : 0) | src2->pr0_reg;
  if (dest->pr0_reg != PREG_REG_NONE && (dest->pr1_reg == PREG_REG_NONE || dest_pr1_packed == dest_pr0_packed))
  {
    if (dest->vr == src1->vr && src1->pr1_reg != PREG_REG_NONE)
    {
      dest->pr1_reg = src1->pr1_reg;
      dest->pr1_spilled = src1->pr1_spilled;
    }
    else if (dest->vr == src2->vr && src2->pr1_reg != PREG_REG_NONE)
    {
      dest->pr1_reg = src2->pr1_reg;
      dest->pr1_spilled = src2->pr1_spilled;
    }
    else if (dest_pr0_packed == src1_pr0_packed && src1->pr1_reg != PREG_REG_NONE)
    {
      dest->pr1_reg = src1->pr1_reg;
      dest->pr1_spilled = src1->pr1_spilled;
    }
    else if (dest_pr0_packed == src2_pr0_packed && src2->pr1_reg != PREG_REG_NONE)
    {
      dest->pr1_reg = src2->pr1_reg;
      dest->pr1_spilled = src2->pr1_spilled;
    }
  ...
  if (src1_is_imm && src2_is_imm)
  {
    ...
    int is_64bit = tcc_is_64bit_operand(dest);
    int pr1_for_const = dest->pr1_spilled ? PREG_REG_NONE : dest->pr1_reg;
    tcc_machine_load_constant(dest->pr0_reg, pr1_for_const, folded_value, is_64bit, NULL);
    return;
  }
  ...
  if (src1_is_imm || src2_is_imm)
  {
    const SValue *reg_src = src1_is_imm ? src2 : src1;
    ...
    /* Materialize the register source if it isn't already in registers (e.g. stack local).
     * Also: a 64-bit stack local may sometimes arrive as VT_LOCAL without VT_LVAL;
     * for 64-bit ops we always need the VALUE, not the address.
     */
    ...
    int pr1_reg_or_none = reg_src->pr1_spilled ? PREG_REG_NONE : reg_src->pr1_reg;
    int rn_high = (reg_src_is64 ? pr1_reg_or_none : PREG_REG_NONE);

    const int reg_src_kind = reg_src->r & VT_VALMASK;
    const bool reg_src_is_stack_local = (reg_src_kind == VT_LOCAL || reg_src_kind == VT_LLOCAL);
    const bool reg_src_is_lval = (reg_src->r & VT_LVAL) != 0;
    const bool reg_src_is_sym = (reg_src->r & VT_SYM) != 0;
    /* Heuristic fallback: some IR paths may lose VT_LOCAL/VT_LLOCAL while still
      carrying a stack frame offset in c.i. For 64-bit ops, treat that as a
      stack value that must be loaded (not an address). */
    const bool reg_src_force_value_load = reg_src_is64 && !(reg_src->r & VT_LVAL) && (reg_src->c.i != 0);
    const bool reg_src_needs_load = reg_src_is64 && (reg_src_is_stack_local || reg_src_is_lval || reg_src_is_sym);

    if (rn_low == PREG_REG_NONE || (reg_src_is64 && rn_high == PREG_REG_NONE) || reg_src_needs_load ||
        reg_src_force_value_load)
    {
      uint32_t load_exclude = 0;
      reg_src_lo_alloc = get_scratch_reg_with_save(load_exclude);
      rn_low = reg_src_lo_alloc.reg;
      if (thumb_is_hw_reg(rn_low))
        load_exclude |= (1u << rn_low);
      if (reg_src_is64)
      {
        reg_src_hi_alloc = get_scratch_reg_with_save(load_exclude);
        rn_high = reg_src_hi_alloc.reg;
        if (thumb_is_hw_reg(rn_high))
          load_exclude |= (1u << rn_high);
      }

      SValue tmp = *reg_src;

      /* For 64-bit logical ops we always need an rvalue. Stack locals sometimes
       * arrive as VT_LOCAL/VT_LLOCAL without VT_LVAL (i.e. address-of stack slot);
       * force a value load in that case.
       */
      if (reg_src_is64)
      {
        const int tmp_kind = tmp.r & VT_VALMASK;
        if ((tmp_kind == VT_LOCAL || tmp_kind == VT_LLOCAL) && !(tmp.r & VT_LVAL))
          tmp.r |= VT_LVAL;
      }

      if (debug_logical64 && reg_src_is64)
      {
        const int reg_src_kind_dbg = reg_src->r & VT_VALMASK;
        const int tmp_kind_dbg = tmp.r & VT_VALMASK;
        uint8_t pr0_packed_dbg = (reg_src->pr0_spilled ? PREG_SPILLED : 0) | reg_src->pr0_reg;
        uint8_t pr1_packed_dbg = (reg_src->pr1_spilled ? PREG_SPILLED : 0) | reg_src->pr1_reg;
        fprintf(stderr,
                "[DBG] %s imm64=0x%08x%08x reg_src.r=0x%x kind=%d LVAL=%d pr0=%d pr1=%d c.i=%lld tmp.r=0x%x kind=%d "
                "LVAL=%d c.i=%lld\n",
                ctx, (unsigned)(imm64 >> 32), (unsigned)(imm64 & 0xffffffffu), reg_src->r, reg_src_kind_dbg,
                !!(reg_src->r & VT_LVAL), pr0_packed_dbg, pr1_packed_dbg, (long long)reg_src->c.i, tmp.r, tmp_kind_dbg,
                !!(tmp.r & VT_LVAL), (long long)tmp.c.i);
      }
      load_to_reg(rn_low, reg_src_is64 ? rn_high : PREG_NONE, &tmp);
    }

    thumb_require_materialized_reg(ctx, "src.low", rn_low);
    if (reg_src_is64 && rn_high != PREG_NONE)
      thumb_require_materialized_reg(ctx, "src.high", rn_high);
    else
      thumb_ensure_not_spilled(ctx, "src.high", rn_high);
  ...
  const bool src1_lo_spilled = (src1->pr0_reg != PREG_REG_NONE) && src1->pr0_spilled;
  const bool src1_hi_spilled = (src1->pr1_reg != PREG_REG_NONE) && src1->pr1_spilled;
  const bool src2_lo_spilled = (src2->pr0_reg != PREG_REG_NONE) && src2->pr0_spilled;
  const bool src2_hi_spilled = (src2->pr1_reg != PREG_REG_NONE) && src2->pr1_spilled;
  const int src1_kind = src1->r & VT_VALMASK;
  const int src2_kind = src2->r & VT_VALMASK;
  const bool src1_is_stack_local = (src1_kind == VT_LOCAL || src1_kind == VT_LLOCAL);
  const bool src2_is_stack_local = (src2_kind == VT_LOCAL || src2_kind == VT_LLOCAL);
  const bool src1_needs_load = src1_is64 && (src1_is_stack_local || (src1->r & VT_LVAL) || (src1->r & VT_SYM));
  const bool src2_needs_load = src2_is64 && (src2_is_stack_local || (src2->r & VT_LVAL) || (src2->r & VT_SYM));
  ...
  if (src1_lo == PREG_REG_NONE || src1_lo_spilled || src1_needs_load)
  {
    ...
    SValue src1_tmp = *src1;
    if (src1_is_stack_local && !(src1_tmp.r & VT_LVAL))
      src1_tmp.r |= VT_LVAL;
    load_to_reg(src1_lo, PREG_NONE, &src1_tmp);
  }
  else
  {
  ...
  if (src1_is64 && (src1_hi == PREG_REG_NONE || src1_hi_spilled || src1_needs_load))
  {
    ...
    SValue src1_hi_val = *src1;
    if (src1_is_stack_local && !(src1_hi_val.r & VT_LVAL))
      src1_hi_val.r |= VT_LVAL;
    src1_hi_val.c.i += 4;
  ...
  if (src2_lo == PREG_REG_NONE || src2_lo_spilled || src2_needs_load)
  {
    ...
    SValue src2_tmp = *src2;
    if (src2_is_stack_local && !(src2_tmp.r & VT_LVAL))
      src2_tmp.r |= VT_LVAL;
    load_to_reg(src2_lo, PREG_NONE, &src2_tmp);
  }
  else
  {
  ...
  if (src2_is64 && (src2_hi == PREG_REG_NONE || src2_hi_spilled || src2_needs_load))
  {
    ...
    SValue src2_hi_val = *src2;
    if (src2_is_stack_local && !(src2_hi_val.r & VT_LVAL))
      src2_hi_val.r |= VT_LVAL;
    src2_hi_val.c.i += 4;
                                   ...
                                   thumb_imm_handler_t cross_shift, bool sign_extend_missing_hi, bool arith_right)
{
  ...
  {
    const bool src_lo_spilled = (src_lo != PREG_REG_NONE) && src1->pr0_spilled;
    const bool src_hi_spilled = (src_hi != PREG_REG_NONE) && src1->pr1_spilled;

    /* Low word must be usable as a register input. */
    if (src_lo == PREG_REG_NONE || src_lo_spilled || (src1->r & VT_LVAL) || src1_is_address_of || src1_is_sym_address ||
        th_has_immediate_value(src1->r))
    {
      src_lo_alloc = get_scratch_reg_with_save(exclude);
      src_lo = src_lo_alloc.reg;
      if (thumb_is_hw_reg(src_lo))
        exclude |= (1u << src_lo);
      load_to_reg(src_lo, PREG_NONE, src1);
    }
    else
    {
    ...
    if (src_hi != PREG_REG_NONE && !src_hi_spilled)
    {
      thumb_require_materialized_reg(ctx, "src1.high", src_hi);
      if (thumb_is_hw_reg(src_hi))
        exclude |= (1u << src_hi);
    }
    else if (src_hi_spilled || (src1->r & VT_LVAL) || src1_is_address_of || src1_is_sym_address)
    {
...
thumb_shift64_cleanup:
  thumb_store_dest_pair_if_needed(dest, dst_lo, dst_hi, store_lo, store_hi);
...
static void thumb_emit_regonly_binop32(SValue *src1, SValue *src2, SValue *dest, TccIrOp op,
                                       thumb_regonly3_handler_t emitter, const char *ctx)
{
  ...
  if (rn == PREG_REG_NONE || (src1->r & VT_LVAL))
  {
    rn_alloc = get_scratch_reg_with_save(exclude);
    rn = rn_alloc.reg;
    exclude |= (1u << rn);
    load_to_reg(rn, PREG_NONE, src1);
  }
  else
  {
  ...
  if (rm == PREG_REG_NONE || (src2->r & VT_LVAL))
  {
    rm_alloc = get_scratch_reg_with_save(exclude);
    rm = rm_alloc.reg;
    load_to_reg(rm, PREG_NONE, src2);
  }
  else
  {
...
static void thumb_emit_mod32(SValue *src1, SValue *src2, SValue *dest, TccIrOp op, thumb_regonly3_handler_t div_emitter,
                             const char *ctx)
{
...
static void thumb_emit_mul32(SValue *src1, SValue *src2, SValue *dest, TccIrOp op)
{
...
static void thumb_emit_longmul32x32_to64(SValue *src1, SValue *src2, SValue *dest, TccIrOp op,
                                         thumb_longmul_handler_t emitter, const char *ctx)
{
  ...
  if (rn == PREG_REG_NONE || (src1->r & VT_LVAL) || th_has_immediate_value(src1->r))
  {
    rn_alloc = get_scratch_reg_with_save(exclude);
    rn = rn_alloc.reg;
    exclude |= (1u << rn);
  ...
  if (rm == PREG_REG_NONE || (src2->r & VT_LVAL) || th_has_immediate_value(src2->r))
  {
    rm_alloc = get_scratch_reg_with_save(exclude);
    rm = rm_alloc.reg;
    exclude |= (1u << rm);
...
static void thumb_process_data64_op(SValue *src1, SValue *src2, SValue *dest, TccIrOp op)
{
  ...
  case TCCIR_OP_SHL:
  {
    if (!th_has_immediate_value(src2->r))
  ...
  return thumb_emit_opcode64_imm(src1, src2, dest, op, context, regular_handler, carry_handler);
}

static void thumb_emit_data_processing_op32(SValue *src1, SValue *src2, SValue *dest, TccIrOp op,
                                            ThumbDataProcessingHandler handler, thumb_flags_behaviour flags)
{
  ...
  const bool src1_is_address_of = ((src1->r & VT_VALMASK) == VT_LOCAL) && !(src1->r & VT_LVAL);
  const bool src2_is_address_of = ((src2->r & VT_VALMASK) == VT_LOCAL) && !(src2->r & VT_LVAL);

  /* VT_LVAL on a non-local operand means "register holds pointer, dereference it" */
  const bool src1_is_lval = (src1->r & VT_LVAL) && !src1_is_address_of;
  const bool src2_is_lval = (src2->r & VT_LVAL) && !src2_is_address_of;

  const bool src1_needs_load = src1_is_imm || src1_is_address_of || src1_is_lval || src1_reg == PREG_REG_NONE;
  const bool src2_needs_load = src2_is_imm || src2_is_address_of || src2_is_lval || src2_reg == PREG_REG_NONE;
  ...
  if (src2_is_imm)
  {
    ...
    if (handler.imm_handler && ot(handler.imm_handler(dest_reg, src1_reg, src2->c.i, flags, ENFORCE_ENCODING_NONE)))
    {
      if (src1_alloc.reg != 0)
        restore_scratch_reg(&src1_alloc);
      return;
...
void tcc_gen_machine_data_processing_op(SValue *src1, SValue *src2, SValue *dest, TccIrOp op)
{
  ThumbDataProcessingHandler handler;
  ...
  {
    const int is64 = tcc_is_64bit_operand(src1);
    int src_lo = src1->pr0_reg;
    int src_hi = src1->pr1_reg;
    ...
    const int needs_load = th_has_immediate_value(src1->r) || src_lo == PREG_REG_NONE || (src1->r & VT_LVAL) ||
                           (is64 && src_hi == PREG_REG_NONE);
    ...
    if (needs_load)
    {
      ...
      load_to_reg(src_lo, src_hi, src1);
    }
    else
    {
...
static int load_fp_operand_to_vfp(SValue *sv, int scratch_sreg, int scratch_dreg, int is_double)
{
  ...
  uint8_t pr0_packed = (sv->pr0_spilled ? PREG_SPILLED : 0) | sv->pr0_reg;
  if (pr0_packed != PREG_NONE && LS_IS_VFP_REG(sv->pr0_reg))
  {
    return LS_VFP_REG_NUM(sv->pr0_reg);
  }
...
static void store_fp_result_from_vfp(SValue *dest, int result_sreg, int result_dreg, int is_double)
{
  ...
  uint8_t pr0_packed = (dest->pr0_spilled ? PREG_SPILLED : 0) | dest->pr0_reg;
  if (pr0_packed != PREG_NONE && LS_IS_VFP_REG(dest->pr0_reg))
  {
    int dest_sreg = LS_VFP_REG_NUM(dest->pr0_reg);
    if (is_double)
    {
  ...
  if (is_double)
  {
    ...
    if (dest->pr0_reg != PREG_REG_NONE || dest->pr1_reg != PREG_REG_NONE)
    {
      ...
      if (dest->pr0_spilled || dest->pr1_spilled)
        tcc_error("compiler_error: hard-float double result destination unexpectedly spilled");
      thumb_require_materialized_reg("store_fp_result_from_vfp", "dest.low", dest->pr0_reg);
      thumb_require_materialized_reg("store_fp_result_from_vfp", "dest.high", dest->pr1_reg);
  ...
  else
  {
    ...
    {
      uint8_t pr0_packed = (dest->pr0_spilled ? PREG_SPILLED : 0) | dest->pr0_reg;
      thumb_require_materialized_reg("store_fp_result_from_vfp", "dest", pr0_packed);
      if (dest->pr0_reg != R0)
      {
        ot_check(th_mov_reg(dest->pr0_reg, R0, FLAGS_BEHAVIOUR_NOT_IMPORTANT, THUMB_SHIFT_DEFAULT,
                            ENFORCE_ENCODING_NONE, false));
 ...
 */
static void gen_hardfp_op(SValue *src1, SValue *src2, SValue *dest, TccIrOp op, int is_double)
{
  ...
  if (op != TCCIR_OP_FNEG)
  {
    src2_reg = load_fp_operand_to_vfp(src2, 2 /* S2 */, 1 /* D1 */, is_double);
  }
  else
  {
 ...
 */
static void gen_hardfp_cmp(SValue *src1, SValue *src2, SValue *dest, TccIrOp op, int is_double)
{
  uint32_t sz = is_double ? 1 : 0;
  int src1_reg, src2_reg;
  ...
  src2_reg = load_fp_operand_to_vfp(src2, scratch_s, scratch_d, is_double);
...
/* Generate a soft-float library function call for FP operations.
 * Parameters are passed in R0, R1 (float) or R0:R1, R2:R3 (double)
 * according to EABI soft-float calling convention.
 */
static void gen_softfp_call(SValue *src1, SValue *src2, SValue *dest, TccIrOp op, const char *func_name, int is_double)
{
  Sym *sym;
  SValue func_sv;
  ...
  if (op == TCCIR_OP_FNEG)
  {
    ...
    load_to_reg(R0, is_double ? R1 : PREG_NONE, src1);
  }
  else if (op == TCCIR_OP_FCMP)
  {
    ...
    if (is_double)
    {
      load_to_reg(R0, R1, src1);
      load_to_reg(R2, R3, src2);
    }
    else
    {
      load_to_reg(R0, PREG_NONE, src1);
      load_to_reg(R1, PREG_NONE, src2);
    }
  }
  else if (op == TCCIR_OP_CVT_FTOF || op == TCCIR_OP_CVT_ITOF || op == TCCIR_OP_CVT_FTOI)
  {
    ...
    int src_is_64bit = is_64bit_type(src1->type.t);
    load_to_reg(R0, src_is_64bit ? R1 : PREG_NONE, src1);
  }
  else
  {
  ...
  if (op != TCCIR_OP_FCMP)
  {
    if (is_64bit_type(dest->type.t))
    {
      ...
      {
        /* Memory destination: store both words using store(). */
        int saved_pr1 = dest->pr1_reg;
        dest->pr1_reg = R1;
        store(R0, dest);
        dest->pr1_reg = saved_pr1;
      }
    }
    else
    {
 ...
 */
static void gen_hardfp_cvt_ftof(SValue *src1, SValue *dest, TccIrOp op)
{
  int src_is_double = ((src1->type.t & VT_BTYPE) != VT_FLOAT);
  int dst_is_double = ((dest->type.t & VT_BTYPE) != VT_FLOAT);
  int src_reg;

  /* Load source - may already be in VFP register */
  src_reg = load_fp_operand_to_vfp(src1, 0 /* S0 */, 0 /* D0 */, src_is_double);
  ...
  store_fp_result_from_vfp(dest, 0 /* S0 */, 0 /* D0 */, dst_is_double);
}

/* Generate int-to-float conversion.
 * Uses VCVT for hard float, library calls for soft float.
 */
static void gen_hardfp_cvt_itof(SValue *src1, SValue *dest, TccIrOp op)
{
  int src_bt = src1->type.t & VT_BTYPE;
  int dst_is_double = ((dest->type.t & VT_BTYPE) != VT_FLOAT);
  ...
  if (src_bt == VT_LLONG)
  {
    ...
    gen_softfp_call(src1, NULL, dest, op, func_name, 0);
    return;
  }
 ...
 * Uses VCVT for hard float, library calls for soft float.
 */
static void gen_hardfp_cvt_ftoi(SValue *src1, SValue *dest, TccIrOp op)
{
  int src_is_double = ((src1->type.t & VT_BTYPE) != VT_FLOAT);
  int dst_bt = dest->type.t & VT_BTYPE;
  int is_unsigned = (dest->type.t & VT_UNSIGNED) ? 1 : 0;
  int src_reg;
  ...
  if (dst_bt == VT_LLONG)
  {
    ...
    gen_softfp_call(src1, NULL, dest, op, func_name, src_is_double);
    return;
  }
...
{
  int is_double = ((src1->type.t & VT_BTYPE) != VT_FLOAT);
  const char *func_name;

  /* Use VFP hardware instructions when:
   * - Hardware float ABI (pass args in VFP regs) OR softfp ABI (pass args in GPRs but can use VFP instructions)
   * - AND the FPU supports the precision (always for float, check for double)
   * Note: For softfp, we use VFP instructions but function calls still pass args in R0-R3 */
  int use_vfp = (tcc_state->float_abi == ARM_HARD_FLOAT || tcc_state->float_abi == ARM_SOFTFP_FLOAT) &&
                (!is_double || arm_fpu_supports_double(tcc_state->fpu_type));
  ...
  if (use_vfp)
  {
    ...
    if (op == TCCIR_OP_FADD || op == TCCIR_OP_FSUB || op == TCCIR_OP_FMUL || op == TCCIR_OP_FDIV || op == TCCIR_OP_FNEG)
    {
  ...
  if (op == TCCIR_OP_CVT_FTOF)
  {
    ...
    else
    {
      ...
      load_to_reg(R0, src_is_double ? R1 : PREG_NONE, src1);
      store(R0, dest);
      return;
    }
    gen_softfp_call(src1, src2, dest, op, func_name_cvt, src_is_double);
    return;
  }
  ...
  if (op == TCCIR_OP_CVT_ITOF)
  {
    ...
    gen_softfp_call(src1, src2, dest, op, func_name_cvt, 0);
    return;
  }
...
{
  int is_64bit = is_64bit_type(src1->type.t);
   ...
   * fields. */
  if ((src1->r & VT_VALMASK) == VT_CONST)
  {
    Sym *sym = (src1->r & VT_SYM) ? src1->sym : NULL;
    tcc_machine_load_constant(R0, is_64bit ? R1 : PREG_NONE, src1->c.i, is_64bit, sym);
    return;
  }
  ...
  if (src1->pr0_reg != PREG_REG_NONE)
  {
    ...
    fprintf(stderr, "DEBUG tcc_gen_machine_return_value_op: pr0_reg=%d pr1_reg=%d is_64bit=%d\n", src1->pr0_reg,
            src1->pr1_reg, is_64bit);
    load_to_register(R0, src1->pr0_reg, src1);
    if (is_64bit && src1->pr1_reg != PREG_REG_NONE)
    {
      if (src1->pr1_spilled)
        tcc_error("compiler_error: return value source high half unexpectedly still spilled");
      load_to_register(R1, src1->pr1_reg, src1);
    }
    return;
  }
...
ST_FUNC void tcc_gen_machine_store_op(SValue *src, SValue *dest, TccIrOp op)
{
   ...
   * long */
  int src_btype = src->type.t & VT_BTYPE;
  ...
  const int src_is_const = ((src->r & VT_VALMASK) == VT_CONST);
  const int src_is_lval = (src->r & VT_LVAL) != 0;
  ...
  if (need_reload)
  {
    ...
    load_to_reg(src_reg, is_64bit ? R11 : PREG_NONE, src);

    SValue store_dest = *dest;
    if (is_64bit)
    {
      store_dest.pr1_reg = R11;
      store_dest.pr1_spilled = 0;
    }
  ...
  else
  {
    ...
    if (is_64bit)
    {
      store_dest.pr1_reg = src->pr1_reg;
      store_dest.pr1_spilled = src->pr1_spilled;
      uint8_t pr1_packed = (store_dest.pr1_spilled ? PREG_SPILLED : 0) | store_dest.pr1_reg;
...
ST_FUNC void tcc_gen_machine_assign_op(SValue *src1, SValue *dest, TccIrOp op)
{
  ...
  int dest_is_vfp = use_vfp_regs && LS_IS_VFP_REG(dest->pr0_reg);
  int src_is_vfp = use_vfp_regs && LS_IS_VFP_REG(src1->pr0_reg);
   ...
   */
  const int dest_is_64bit = is_64bit_type(dest->type.t);
  const int src_is_64bit = is_64bit_type(src1->type.t);

  fprintf(stderr, "DEBUG assign_op: src1->vr=%d src1->pr0_reg=%d src1->pr1_reg=%d src1->type.t=0x%x src_is_64=%d\n",
          src1->vr, src1->pr0_reg, src1->pr1_reg, src1->type.t, src_is_64bit);
  fprintf(stderr, "DEBUG assign_op: dest->vr=%d dest->pr0_reg=%d dest->pr1_reg=%d dest->type.t=0x%x dest_is_64=%d\n",
          dest->vr, dest->pr0_reg, dest->pr1_reg, dest->type.t, dest_is_64bit);
  ...
  if (dest_is_64bit)
  {
    ...
    const int dest_in_mem = (dest->r & VT_LVAL) != 0;

    int src_lo = src1->pr0_reg;
    int src_hi = src_is_64bit ? src1->pr1_reg : PREG_REG_NONE;
    ...
    const int src_lo_spilled = (src_lo != PREG_REG_NONE) && src1->pr0_spilled;
    const int src_hi_spilled = (src_hi != PREG_REG_NONE) && src1->pr1_spilled;
    ...
    if ((src1->r & VT_VALMASK) == VT_CONST || (src1->r & VT_LVAL) || src_lo == PREG_REG_NONE || src_lo_spilled ||
        (src_is_64bit && src_hi_spilled))
    {
      ...
      {
        src_hi_alloc = get_scratch_reg_with_save(exclude);
        load_to_reg(src_lo_alloc.reg, src_hi_alloc.reg, src1);
        src_hi = src_hi_alloc.reg;
      }
      else
      {
        load_to_reg(src_lo_alloc.reg, PREG_REG_NONE, src1);
  ...
  if ((src1->r & VT_VALMASK) == VT_CONST && !(src1->r & VT_LVAL))
  {
    ...
    if (dest_is_vfp)
    {
      int dn = LS_VFP_REG_NUM(dest->pr0_reg);
      /* Load constant to integer register, then move to VFP */
      ScratchRegAlloc scratch_alloc = get_scratch_reg_with_save(0);
      int scratch_reg = scratch_alloc.reg;
      tcc_machine_load_constant(scratch_reg, PREG_NONE, src1->c.i, 0, sym);
      ot_check(th_vmov_gp_sp(scratch_reg, dn, 0)); /* VMOV Sn, scratch_reg */
      restore_scratch_reg(&scratch_alloc);
    }
    else if ((dest->r & VT_LVAL) && ((dest->r & VT_VALMASK) == VT_LOCAL || (dest->r & VT_VALMASK) == VT_CONST))
    {
    ...
    {
      tcc_machine_load_constant(dest->pr0_reg, is_64bit ? dest->pr1_reg : PREG_REG_NONE, src1->c.i, is_64bit, sym);
    }
    return;
  }
  ...
  if ((src1->r & VT_VALMASK) == VT_CONST && (src1->r & VT_LVAL))
  {
    if (dest_is_vfp)
    {
      ...
      ScratchRegAlloc scratch_alloc = get_scratch_reg_with_save(0);
      int scratch_reg = scratch_alloc.reg;
      load_to_reg(scratch_reg, PREG_REG_NONE, src1);
    ...
    {
      load_to_reg(dest->pr0_reg, dest_is_64bit ? dest->pr1_reg : PREG_REG_NONE, src1);
    }
    return;
  }
  ...
  if ((src1->r & VT_VALMASK) == VT_LOCAL)
  {
    load_to_dest(dest, src1);
    return;
  }

  if (dest->pr0_reg == src1->pr0_reg && dest->pr0_spilled == src1->pr0_spilled)
    return;
  ...
  {
    int dn = LS_VFP_REG_NUM(dest->pr0_reg);
    int sn = LS_VFP_REG_NUM(src1->pr0_reg);
    /* VFP to VFP move */
  ...
  else if (dest_is_vfp && !src_is_vfp)
  {
    int dn = LS_VFP_REG_NUM(dest->pr0_reg);
    /* Integer to VFP */
    ot_check(th_vmov_gp_sp(src1->pr0_reg, dn, 0)); /* VMOV Sn, Rm */
  }
  else if (!dest_is_vfp && src_is_vfp)
  {
    int sn = LS_VFP_REG_NUM(src1->pr0_reg);
    /* VFP to integer */
    ot_check(th_vmov_gp_sp(dest->pr0_reg, sn, 1)); /* VMOV Rd, Sn */
  }
  else
  {
    /* Integer to integer */
    fprintf(stderr, "DEBUG assign_op i2i: emitting MOV R%d, R%d (src_is_64=%d dest_is_64=%d)\n", dest->pr0_reg, src1->pr0_reg, src_is_64bit, dest_is_64bit);
    if (src_is_64bit && !dest_is_64bit) {
      fprintf(stderr, "DEBUG assign_op i2i: 64->32 NARROWING, using pr0_reg=%d from src1\n", src1->pr0_reg);
    }
    ot_check(th_mov_reg(dest->pr0_reg, src1->pr0_reg, FLAGS_BEHAVIOUR_NOT_IMPORTANT, THUMB_SHIFT_DEFAULT,
                        ENFORCE_ENCODING_NONE, false));
  }
...
ST_FUNC void tcc_gen_machine_lea_op(SValue *src1, SValue *dest, TccIrOp op)
{
  ...
  if (src_v == VT_LOCAL || src_v == VT_LLOCAL)
  {
     ...
     */
    int offset;
    const int vreg_type = TCCIR_DECODE_VREG_TYPE(src1->vr);
    if (vreg_type == TCCIR_VREG_TYPE_VAR && src1->c.i != 0)
    {
    ...
    else
    {
      ...
      const TCCStackSlot *slot = tcc_ir_stack_slot_by_vreg(tcc_state->ir, src1->vr);
      if (slot)
        offset = slot->offset;
      else
        offset = (int)src1->c.i;
  ...
  else if (src_v == VT_CONST && (src1->r & VT_SYM))
  {
    /* Address of global symbol */
    load_full_const(dest_reg, PREG_NONE, src1->c.i, src1->sym);
  }
  else
  {
    /* Fallback: if src is already in a register, just move it */
    int src_reg = src1->pr0_reg;
    if (src_reg != PREG_REG_NONE)
    {
    ...
    {
      tcc_error("compiler_error: LEA on unexpected operand type r=0x%x", src1->r);
    }
  }
}
...
static void thumb_emit_arg_move(const ThumbArgMove *m)
{
  ...
  if (m->kind == THUMB_ARG_MOVE_STRUCT)
  {
    ...
    else if (sv_copy.pr0_reg != PREG_REG_NONE && !sv_copy.pr0_spilled)
    {
      /* Address already in a register */
      base_addr_reg = sv_copy.pr0_reg;
    }
    else
    {
                                          ...
                                          TCCIRState *ir, int call_idx)
{
  ...
  /* Get call_id and argc from call_id_sv.c.i (keeps call/param binding explicit). */
  ...
  for (int i = 0; i < argc; ++i)
  {
    ...
    /* Handle register arguments (including the register part of split args) */
    if (loc->kind != TCC_ABI_LOC_REG && loc->kind != TCC_ABI_LOC_REG_STACK)
      continue;

    int base_reg = ARM_R0 + loc->reg_base;
    fprintf(stderr, "DEBUG CALLER: arg %d bt=%d loc->kind=%d base_reg=%d reg_count=%d\n", i, bt, loc->kind, base_reg,
            loc->reg_count);

    if (bt == VT_STRUCT)
    {
      /* Load struct words into consecutive registers.
       * For TCC_ABI_LOC_REG_STACK, only load the first reg_count words to registers. */
      int words = loc->reg_count; /* Use reg_count, not total size */
      fprintf(stderr, "DEBUG CALLER STRUCT: adding THUMB_ARG_MOVE_STRUCT dst_reg=%d word_count=%d\n", base_reg, words);
      if (words > 0 && words <= 4)
      {
      ...
      else if (arg->pr0_reg != PREG_REG_NONE && arg->pr1_reg != PREG_REG_NONE)
      {
        if (arg->pr0_reg != base_reg)
          reg_moves[reg_move_count++] =
              (ThumbArgMove){.kind = THUMB_ARG_MOVE_REG, .dst_reg = base_reg, .src_reg = arg->pr0_reg};
    ...
    fprintf(stderr, "DEBUG CALLER 32-bit arg %d: pr0_reg=%d pr0_spilled=%d r=0x%x VT_LVAL=%d\n", 
            i, arg->pr0_reg, arg->pr0_spilled, arg->r, (arg->r & VT_LVAL) ? 1 : 0);
    if (arg->r & VT_LVAL)
    {
    ...
    else if (arg->pr0_reg != PREG_REG_NONE && !arg->pr0_spilled)
    {
      if (arg->pr0_reg != base_reg)
      {
        fprintf(stderr, "DEBUG CALLER: adding THUMB_ARG_MOVE_REG dst_reg=%d src_reg=%d\n", base_reg, arg->pr0_reg);
        reg_moves[reg_move_count++] =
            (ThumbArgMove){.kind = THUMB_ARG_MOVE_REG, .dst_reg = base_reg, .src_reg = arg->pr0_reg};
      }
    }
  ...
  for (int i = 0; i < argc; ++i)
  {
    ...
    if (arg->pr0_reg != PREG_REG_NONE && !arg->pr0_spilled && arg->pr0_reg <= ARM_R3)
    {
      ...
      if (!store_word_to_base(arg->pr0_reg, ARM_SP, stack_offset, 0))
      {
        load_immediate(ARM_R12, stack_offset, NULL, false);
        ot_check(th_str_reg(arg->pr0_reg, ARM_SP, ARM_R12, THUMB_SHIFT_DEFAULT, ENFORCE_ENCODING_NONE));
      }
    }
  }
  ...
  for (int i = 0; i < argc; ++i)
  {
    ...
    fprintf(stderr, "DEBUG STACK ARG %d: bt=%d r=0x%x pr0_reg=%d pr0_spilled=%d c.i=%lld loc->stack_off=%d\n", i, bt,
            arg->r, arg->pr0_reg, arg->pr0_spilled, (long long)arg->c.i, loc->stack_off);

    /* Handle TCC_ABI_LOC_STACK and TCC_ABI_LOC_REG_STACK (stack portion) */
    {
      /* Argument goes (partially) on stack */
      int stack_offset = loc->stack_off;
      /* For split args, skip words that went to registers */
      int words_in_regs = (loc->kind == TCC_ABI_LOC_REG_STACK) ? loc->reg_count : 0;
      int struct_src_offset = words_in_regs * 4; /* Offset into struct for stack portion */

      if (bt == VT_STRUCT)
      {
        /* Copy struct to stack. Get struct address and copy each word.
         * For TCC_ABI_LOC_REG_STACK, only copy the words that didn't fit in registers. */
      ...
      else if (is_64bit)
      {
        ...
        else if (arg->pr0_reg != PREG_REG_NONE && arg->pr1_reg != PREG_REG_NONE)
        {
          ...
          int lo_reg = arg->pr0_reg;
          int hi_reg = arg->pr1_reg;
      ...
      else
      {
        ...
        else if ((arg->r & VT_VALMASK) == VT_CONST)
        {
          ...
          {
            /* R12 holds the value; use a different register for the offset. */
  ...
  if (!drop_value)
  {
    fprintf(stderr, "DEBUG func_call Step8: call_id=%d dest->vr=%d dest->pr0_reg=%d dest->pr0_spilled=%d dest->pr1_reg=%d dest->type.t=0x%x\n",
            call_id, dest->vr, dest->pr0_reg, dest->pr0_spilled, dest->pr1_reg, dest->type.t);
    /* Move return value from R0 (and R1 for 64-bit) to destination */
    if (dest->pr0_reg != PREG_REG_NONE && dest->pr0_reg != ARM_R0)
    {
      fprintf(stderr, "DEBUG func_call Step8: emitting MOV R%d, R0\n", dest->pr0_reg);
      ot_check(th_mov_reg(dest->pr0_reg, ARM_R0, FLAGS_BEHAVIOUR_NOT_IMPORTANT, THUMB_SHIFT_DEFAULT,
                          ENFORCE_ENCODING_NONE, false));
    }
    else if (dest->pr0_reg == ARM_R0)
    {
      fprintf(stderr, "DEBUG func_call Step8: result already in R0, no MOV needed\n");
    }
    else
    {
      fprintf(stderr, "DEBUG func_call Step8: WARNING - dest->pr0_reg=31, result may be lost!\n");
    }

    if (tcc_is_64bit_type(dest->type.t) && dest->pr1_reg != PREG_REG_NONE)
    {
      if (dest->pr1_reg != ARM_R1)
      {
        ot_check(th_mov_reg(dest->pr1_reg, ARM_R1, FLAGS_BEHAVIOUR_NOT_IMPORTANT, THUMB_SHIFT_DEFAULT,
                            ENFORCE_ENCODING_NONE, false));
      }
    }
  }
...
ST_FUNC void tcc_gen_machine_vla_op(SValue *src1, SValue *src2, SValue *dest, TccIrOp op)
{
  ...
  case TCCIR_OP_VLA_ALLOC:
  {
    ...
    if (r == PREG_NONE || (src1->r & VT_VALMASK) == VT_CONST)
    {
      r = R_IP;
      load_to_reg(r, PREG_NONE, src1);
    }
...
ST_FUNC void tcc_gen_machine_func_parameter_op(SValue *src1, SValue *src2, TccIrOp op)
{
  ...
  int call_id = TCCIR_DECODE_CALL_ID(src2->c.i);
  int param_index = TCCIR_DECODE_PARAM_IDX(src2->c.i);
