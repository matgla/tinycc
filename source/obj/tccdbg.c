...
/* Late-reopt compaction support (erase_text_range in tccgen.c): the DWARF
 * line program is buffered in dwarf_line.line_data until the flush in
 * tcc_debug_end, so the erase's walk over flushed reloc sections never
 * sees its DW_LNE_set_address anchors — leaving the first body's rows to
 * alias the re-emitted body once the section tail shifts down.  Patch the
 * buffered anchors here, mirroring the flushed-reloc rules: anchors past
 * the erased range shift down with the tail; anchors inside it belong to
 * the first compile and are retargeted to `tomb_sym` (a local SHN_ABS
 * symbol at 0xFF000000) with the intra-body offset preserved, so the dead
 * sequence stays monotone and never overlaps real addresses.  Rows that
 * follow an anchor advance by in-opcode deltas and ride along with it.
 * Finally, if the erased section is the line state's current section,
 * force a fresh set_address for the next row: a delta from the erased
 * body's last_pc would anchor the re-emitted code to stale offsets. */
                                        ...
                                        int tomb_sym)
{
  ...
  ElfW(Sym) * syms;
  int num_syms;
  if (!s1->dwarf || !symtab_section || !symtab_section->data)
    return;
  syms = (ElfW(Sym) *)symtab_section->data;
  num_syms = symtab_section->data_offset / sizeof(ElfW(Sym));
  for (i = 0; i < n_dwarf_line_relocs; i++)
  {
    struct dwarf_line_reloc_struct *lr = &dwarf_line_relocs[i];
    addr_t a;
    if (lr->sym_index <= 0 || lr->sym_index >= num_syms)
      continue;
    if (syms[lr->sym_index].st_shndx != sec->sh_num ||
        ELFW(ST_TYPE)(syms[lr->sym_index].st_info) != STT_SECTION)
      continue;
#if SHT_RELX == SHT_RELA
    a = (addr_t)lr->addend;
    if (a >= start + size)
    ...
    {
      lr->sym_index = tomb_sym;
      lr->addend = a - start;
    }
#else
    if (lr->line_data_offset + PTR_SIZE > dwarf_line.line_size)
      continue;
    a = read32le(dwarf_line.line_data + lr->line_data_offset);
    if (a >= start + size)
      write32le(dwarf_line.line_data + lr->line_data_offset, a - shift);
    else if (a >= start)
    {
      lr->sym_index = tomb_sym;
      write32le(dwarf_line.line_data + lr->line_data_offset, (uint32_t)(a - start));
    }
#endif
  }
  if (dwarf_line.cur_section == sec)
    dwarf_line.cur_section = NULL;
}
