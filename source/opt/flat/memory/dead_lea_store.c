...
/* Eliminates STOREs to a stack slot in the temp-deref form
 *     T0 <-- Addr[StackLoc[-4]] ;  T0***DEREF*** <-- T2 [STORE]
 * (what remains after known_bits collapses bitfield extract chains) when no
 * later instruction reads that slot.  This approximates per-slot escape
 * analysis and bails wide so it stays obviously sound: it gives up on
 * functions with IJUMP/SETJMP/LONGJMP/INLINE_ASM/VLA_ALLOC or a nested-frame
 * pointer, on any address escaping past STORE-dest/CMP-src/ASSIGN-LEA
 * propagation/mem* PARAM with constant size, and on any CALL that is not a
 * recognized mem* helper (a generic call may deref an escaped address). */
...
int tcc_ir_opt_dead_lea_store_elim(TCCIRState *ir)
{
  ...
  /* Collect loop back-edges (JUMP/JUMPIF to an earlier-or-equal position).
   * Pass 3's liveness is position-based (`read.pos > store.pos`), sound only in
   * straight-line code: inside a loop a store whose slot is read elsewhere in
   * the same body is loop-carried-live even when every read is at an EARLIER
   * position (it re-executes next iteration via the back-edge). Without this a
   * `c.v--` write-back in `while (c.v-- > 0)` was wrongly dropped. */
  int be_cap = 8, be_n = 0;
...
#define RESOLVE_LVAL_SLOT(_op)                                              \
   ...
   * Linear list; functions here are small (post-known_bits) so O(reads*stores)
   * is fine. */
  typedef struct
  {
...
#define ADD_READ(off_, width_, pos_)                                        \
  ...
  for (int i = 0; i < n && !bail; i++)
  {
    ...
    /* mem* PARAMs: PARAM0 is a write-only destination (not a read), PARAM1 is
    ...
    if (q->op == TCCIR_OP_FUNCPARAMVAL || q->op == TCCIR_OP_FUNCPARAMVOID)
    {
      ...
      /* PARAM1: locate the matching call and its size param, mark the read. */
      int call_pos = -1;
      int read_size = 0;
      int size_ok = 0;
      for (int j = i + 1; j < n; j++)
      ...
      /* Resolve the source operand to a stack slot, if it is one. */
      int32_t off;
      int got_off = 0;
      ...
      {
        /* mem* source is a global/static symbol address: it reads that object,
         * never a tracked stack slot, so it is not a read of any local (the
         * `local = global_struct;` init copy). Without this the pass bailed on
         * the whole function and left a store to a since-dead local alive. */
      }
      else
      {
    ...
    {
      /* STOREs are eliminated in Pass 3; here only bail if src1 (the value) is
       * an address being stored into memory, which would escape it. */
      IROperand s1 = tcc_ir_op_get_src1(ir, q);
      if (s1.is_local && !s1.is_lval && irop_get_tag(s1) == IROP_TAG_STACKOFF)
      {
    ...
    /* Walk operands; record reads of known slots and bail on any non-tame use
     * of a known-address vreg. k==3 is MLA's accumulator: a slot read through
     * Tacc that src1/src2 never surface (struct_byval seed 11651). */
    for (int k = 0; k < 4; k++)
    {
      IROperand op;
      int has;
      ...
      /* Dest role (k=0) is a definition, not a use. */
      if (k == 0)
        continue;

      /* Non-lval address use. Allowed shapes: ASSIGN/LEA into a tracked TEMP
       * with the same off (Pass 1 propagation), CMP either operand, mem* PARAM
       * (handled above). Anything else lets the address escape — bail. */
      int is_direct_addr =
      ...
      case TCCIR_OP_LEA:
        /* Propagate address into another TEMP: allow only if the dest TEMP is
         * tracked with the same slot offset, else the address escapes into an
         * untracked vreg/VAR/PARAM whose reads we might miss — bail. */
  ...
  for (int i = 0; i < n; i++)
  {
     ...
     * overwrites S1's byte range with no read of those bytes between, S1 is
     * never observed even if the slot is read further on (that read sees the
     * covering store). Restricting to a straight-line run (break at any
     * control-flow op or jump target) keeps it sound: the covering store
     * unconditionally runs after S1 before any branch could route to a read.
     * Intermediate stores never read S1 (escape-checked in Pass 2). */
    int waw_dead = 0;
    for (int j = i + 1; j < n; j++)
    {
      IRQuadCompact *qj = &ir->compact_instructions[j];
    ...
    for (int r = 0; r < reads_n; r++)
    {
      /* Byte-range overlap of the read with [store_off, store_off+dest_w):
       * only stores whose bytes are never read later may be eliminated. */
      if (!(store_off < reads[r].off + reads[r].width &&
            reads[r].off < store_off + dest_w))
        continue;
      if (reads[r].pos > i)
