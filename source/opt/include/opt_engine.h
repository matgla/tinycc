...
typedef struct IROptCtx
{
  ...
  /* Opaque state for run_stateful_gens passes; NULL for stateless run_gens. */
  void *pass_state;
...
/* begin's return lands in ctx->pass_state; each_pre runs per index before NOP-skip/dispatch; end frees. */
