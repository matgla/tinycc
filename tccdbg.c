...
static int dwarf_arm_thumb_count_bits(uint32_t mask)
{
  int count = 0;
  while (mask)
  {
...
static int dwarf_arm_thumb_offset_words(uint32_t mask, int reg)
{
  int count = 0;
  for (int r = reg + 1; r <= 15; ++r)
  {
    if (mask & (1u << r))
      count++;
  }
...
static void dwarf_arm_thumb_emit_offsets(Section *s, uint32_t mask)
{
  for (int r = 0; r <= 15; ++r)
  {
    if (mask & (1u << r))
    {
      int offset_words = dwarf_arm_thumb_offset_words(mask, r);
      dwarf_data1(s, DW_CFA_offset + r);
      dwarf_uleb128(s, offset_words);
    }
  }
}

static void arm_ehabi_emit_vsp_add(uint8_t *out, int *len, int bytes)
{
  while (bytes > 0)
  {
    int chunk = bytes > 0x100 ? 0x100 : bytes;
    int opcode = (chunk / 4) - 1;
    if (opcode < 0)
      opcode = 0;
    out[(*len)++] = (uint8_t)opcode;
    bytes -= chunk;
  }
}

static void arm_ehabi_write_inline_exidx(Section *exidx, const uint8_t *instr, int len)
{
  uint8_t b0 = len > 0 ? instr[0] : 0xb0;
  uint8_t b1 = len > 1 ? instr[1] : 0xb0;
  uint8_t b2 = len > 2 ? instr[2] : 0xb0;
  uint32_t word = (0x80u << 24) | ((uint32_t)b0 << 16) | ((uint32_t)b1 << 8) | (uint32_t)b2;
  dwarf_data4(exidx, word);
}

static unsigned long arm_ehabi_write_extab(Section *extab, const uint8_t *instr, int len)
{
  unsigned long start = extab->data_offset;
  int word_count = 0;
  uint8_t b2 = 0xb0;
  uint8_t b3 = 0xb0;

  if (len > 2)
    word_count = (len - 2 + 3) / 4;
  if (len > 0)
    b2 = instr[0];
  if (len > 1)
    b3 = instr[1];
  ...
  {
    uint32_t header = (0x81u << 24) | ((uint32_t)word_count << 16) | ((uint32_t)b2 << 8) | (uint32_t)b3;
    dwarf_data4(extab, header);
  }

  {
    ...
    {
      uint8_t d0 = idx < len ? instr[idx++] : 0;
      uint8_t d1 = idx < len ? instr[idx++] : 0;
      uint8_t d2 = idx < len ? instr[idx++] : 0;
      uint8_t d3 = idx < len ? instr[idx++] : 0;
      uint32_t word = ((uint32_t)d0 << 24) | ((uint32_t)d1 << 16) | ((uint32_t)d2 << 8) | (uint32_t)d3;
      dwarf_data4(extab, word);
    }
  }

  dwarf_data4(extab, 0); /* no handler data */
  return start;
}

static void arm_ehabi_emit_function_entry(TCCState *s1)
{
  Section *exidx = s1->arm_exidx_section;
  Section *extab = s1->arm_extab_section;
  uint8_t instr[128];
  int len = 0;
  uint32_t mask = pushed_registers;
  uint32_t mask_r4_r11;
  uint8_t mask8;
  int has_lr;
  int has_r12;
  int need_fp = s1->need_frame_pointer;
  int stack_size = allocated_stack_size;

  if (!exidx || !extab)
    return;

  mask &= ~(1u << 15); /* ignore PC if present */
  mask_r4_r11 = (mask >> 4) & 0xffu;
  mask8 = (uint8_t)mask_r4_r11;
  has_lr = (mask & (1u << 14)) != 0;
  has_r12 = (mask & (1u << 12)) != 0;

  if (need_fp)
  {
    instr[len++] = 0x90u + 11u; /* vsp = r11 */
  }
  else if (stack_size > 0)
  {
    arm_ehabi_emit_vsp_add(instr, &len, stack_size);
  }
  ...
  if (has_r12)
    arm_ehabi_emit_vsp_add(instr, &len, 4);
  ...
  instr[len++] = 0xb0u; /* finish */

  if (len <= 3)
  {
    while (len < 3)
      instr[len++] = 0xb0u;
    arm_ehabi_write_inline_exidx(exidx, instr, len);
  }
  else
  {
    unsigned long extab_off = arm_ehabi_write_extab(extab, instr, len);
    int extab_sym = dwarf_get_section_sym(extab);
    put_elf_reloc(symtab_section, exidx, exidx->data_offset, R_ARM_PREL31, extab_sym);
    dwarf_data4(exidx, extab_off);
  }
}
#endif
