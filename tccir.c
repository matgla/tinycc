...
/* Expand a compact instruction to a full TACQuadruple.
 * This is used during migration to allow existing backend code to work unchanged.
 * The expanded quad reads operands from the pool. */
 ...
 * This is needed when codegen modifies operands (e.g., filling in physical registers). */
                                         ...
                                         int *out_kind, int *out_offset)
{
  ...
  const SValue *def_src1 = tcc_ir_op_get_src1((TCCIRState *)ir, def);
    ...
    if (can_coalesce)
    {
      ...
      *prev_dest = *dest;
       ...
       * dest is always first in the layout when present. */
      {
        ...
        {
          ir->svalue_pool[prev_cq->operand_base] = *prev_dest;
        }
      }
...
static int same_vreg(const SValue *a, const SValue *b)
{
...
static int same_bool_operands(TCCIRState *ir, IRQuadCompact *q1, IRQuadCompact *q2)
{
  ...
  /* Same order: (a,b) == (a,b) */
  if (same_vreg(s11, s21) && same_vreg(s12, s22))
    return 1;
  /* Swapped order: (a,b) == (b,a) - only valid for commutative ops */
  if (same_vreg(s11, s22) && same_vreg(s12, s21))
    return 1;
  return 0;
}
...
int tcc_ir_bool_cse(TCCIRState *ir)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    {
      IRQuadCompact *prev = &ir->compact_instructions[e->instruction_idx];
      if (prev->op == q->op && same_bool_operands(ir, prev, q))
      {
...
int tcc_ir_return_value_optimization(TCCIRState *ir)
{
  ...
  /* Conservative return value optimization: fold
   *   tmp = LOAD [addr]
   *   RETURNVALUE tmp
   * into
   *   RETURNVALUE [addr]
   * when:
   * - tmp is a TEMP vreg
   * - the defining op is a LOAD in the same basic block
   * - tmp is used only by that RETURNVALUE inside the block
   * - the LOAD reads from a stack local (VT_LOCAL)
   * - we can prove the stack slot has been written via a prior STORE in-block
   */
  for (int i = 0; i < n; i++)
  {
    IRQuadCompact *ret = &ir->compact_instructions[i];
    if (ret->op == TCCIR_OP_NOP)
      continue;
    ...
    for (int j = i - 1; j >= 0; --j)
    {
      ...
      if (op == TCCIR_OP_JUMP || op == TCCIR_OP_JUMPIF || op == TCCIR_OP_FUNCCALLVOID || op == TCCIR_OP_FUNCCALLVAL ||
    ...
    /* Find the defining instruction for the returned tmp vreg within the same BB */
    int def_idx = -1;
    for (int j = i - 1; j >= bb_start; --j)
    {
    ...
    IRQuadCompact *def = &ir->compact_instructions[def_idx];
    ...
    /* Check that the temp vreg is only used by this RETURNVALUE within the BB */
    int use_count = 0;
...
int tcc_ir_bool_simplification(TCCIRState *ir)
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    IRQuadCompact *q1 = &ir->compact_instructions[def1];
    IRQuadCompact *q2 = &ir->compact_instructions[def2];
    ...
    if (!same_bool_operands(ir, q1, q2))
      continue;
...
void tcc_ir_generate_code(TCCIRState *ir)
{
  ...
  for (int i = 0; i < ir->next_instruction_index; i++)
  {
    ...
    SValue *src2 = tcc_ir_op_get_src2(ir, cq);
    SValue *dest = tcc_ir_op_get_dest(ir, cq);
    if (irop_config[cq->op].has_src1 == 1)
    {
      tcc_ir_fill_registers(ir, src1);
    }
    if (irop_config[cq->op].has_src2 == 1)
    {
      tcc_ir_fill_registers(ir, src2);
    }
    if (irop_config[cq->op].has_dest == 1)
    {
    ...
    {
      tcc_ir_materialize_addr(ir, src1, &mat_src1_addr, dest->pr0);
    }
    ...
    {
      tcc_ir_materialize_addr(ir, src2, &mat_src2_addr, dest->pr0);
    }
    ...
    case TCCIR_OP_ADC_GEN:
    case TCCIR_OP_ADC_USE:
      tcc_gen_machine_data_processing_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    case TCCIR_OP_FADD:
    case TCCIR_OP_FSUB:
    case TCCIR_OP_FMUL:
    ...
    case TCCIR_OP_LOAD:
    {
      ...
      const SValue *ir_next_src1 = ir_next ? tcc_ir_op_get_src1(ir, ir_next) : NULL;
      if (ir_next && ir_next->op == TCCIR_OP_RETURNVALUE && ir_next_src1 && ir_next_src1->vr == dest->vr &&
          !has_incoming_jump[i + 1])
      {
        dest->pr0 = REG_IRET; /* R0 */
        if (tcc_ir_is_64bit_type(dest->type.t))
        {
          dest->pr1 = REG_IRE2; /* R1 */
        }
      }
      tcc_ir_expand_quad(ir, i, &qq);
      tcc_gen_machine_load_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    }
    case TCCIR_OP_STORE:
      tcc_gen_machine_store_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    case TCCIR_OP_RETURNVALUE:
    {
      ...
      THGEN_DUMP("DEBUG codegen RETURNVALUE: i=%d src1.r=0x%x src1.vr=%d src1.c.i=%lld src1.pr0=%d prev.op=%d "
                 ...
                 i, q->src1.r, src1->vr, (long long)src1->c.i, src1->pr0, ir_prev ? ir_prev->op : -1,
                 ir_prev_dest ? ir_prev_dest->vr : -2, ir_prev_dest ? ir_prev_dest->pr0 : -2);
      if (!has_incoming_jump[i] && ir_prev && (ir_prev->op == TCCIR_OP_LOAD || ir_prev->op == TCCIR_OP_ASSIGN) &&
          ir_prev_dest->vr == src1->vr && ir_prev_dest->pr0 == REG_IRET /* R0 */)
      {
      ...
      {
        tcc_gen_machine_return_value_op(&qq);
        tcc_ir_writeback_quad(ir, i, &qq);
      }
    }
    case TCCIR_OP_RETURNVOID:
    ...
    case TCCIR_OP_ASSIGN:
    {
      ...
      const IRQuadCompact *ir_next = (i + 1 < ir->next_instruction_index) ? &ir->compact_instructions[i + 1] : NULL;
      const SValue *ir_next_src1 = ir_next ? tcc_ir_op_get_src1(ir, ir_next) : NULL;
      if (ir_next && ir_next->op == TCCIR_OP_RETURNVALUE && ir_next_src1->vr == dest->vr && !has_incoming_jump[i + 1])
      {
        dest->pr0 = REG_IRET; /* R0 */
        if (tcc_ir_is_64bit_type(dest->type.t))
          dest->pr1 = REG_IRE2; /* R1 */
      ...
      tcc_ir_expand_quad(ir, i, &qq);
      tcc_gen_machine_assign_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    }
    case TCCIR_OP_LEA:
      /* Load Effective Address: compute address of src1 into dest */
      tcc_gen_machine_lea_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    case TCCIR_OP_FUNCPARAMVAL:
    {
      tcc_gen_machine_func_parameter_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    }
    case TCCIR_OP_JUMP:
      ...
      break;
    case TCCIR_OP_JUMPIF:
      tcc_gen_machine_conditional_jump_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      /* Update mapping to actual instruction address (may have shifted due to literal pool) */
      ir_to_code_mapping[i] = ind - 4;
      ...
      break;
    case TCCIR_OP_IJUMP:
      tcc_gen_machine_indirect_jump_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      tcc_ir_spill_cache_clear(&ir->spill_cache);
      break;
    case TCCIR_OP_SETIF:
      tcc_gen_machine_setif_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    case TCCIR_OP_BOOL_OR:
    case TCCIR_OP_BOOL_AND:
      tcc_gen_machine_bool_op(&qq);
      tcc_ir_writeback_quad(ir, i, &qq);
      break;
    case TCCIR_OP_FUNCPARAMVOID:
    ...
    case TCCIR_OP_FUNCCALLVAL:
    {
      tcc_gen_machine_func_call_op(&qq, drop_return_value, ir, i);
      tcc_ir_writeback_quad(ir, i, &qq);
    ...
    {
#ifdef CONFIG_TCC_ASM
      tcc_ir_codegen_inline_asm(ir, &qq);
      tcc_ir_writeback_quad(ir, i, &qq);
    ...
    tcc_ir_storeback_materialized_dest(&qq, &mat_dest);
    tcc_ir_writeback_quad(ir, i, &qq);
...
void tcc_ir_backpatch_first(TCCIRState *ir, int t, int target_address)
{
  ...
  tcc_ir_get_dest(ir, lp)->c.i = target_address;
}
    ...
    if (jtrue >= 0 || jfalse >= 0)
    {
      ...
      if (jtrue >= 0)
      {
        ...
        if (jfalse >= 0)
        {
          ...
          tcc_ir_get_dest(ir, skip_jump)->c.i = ir->next_instruction_index;
        }
      }
      else if (jfalse >= 0)
      {
      ...
      tcc_ir_get_dest(ir, end_jump)->c.i = ir->next_instruction_index;
