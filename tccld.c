...
static void ld_ungetc(LDParser *p, int c) { p->cc = c; }
...
static addr_t ld_parse_expr(LDParser *p) { return ld_parse_or(p); }
...
static int ld_parse_memory(LDParser *p) {
  ...
  while (p->tok != '}' && p->tok != LDTOK_EOF) {
    if (p->tok == LDTOK_NAME) {
      ...
      /* Parse ORIGIN */
      if (p->tok == LDTOK_NAME &&
          (!strcmp(p->tok_buf, "ORIGIN") || !strcmp(p->tok_buf, "org") ||
           !strcmp(p->tok_buf, "o"))) {
        ld_next_token(p);
        ld_expect(p, '=');
        mr->origin = ld_parse_expr(p);
        printf("Memory region %s: ORIGIN=0x%llx\n", mr->name,
               (unsigned long long)mr->origin);
      }
      ...
      /* Parse LENGTH */
      if (p->tok == LDTOK_NAME &&
          (!strcmp(p->tok_buf, "LENGTH") || !strcmp(p->tok_buf, "len") ||
           !strcmp(p->tok_buf, "l"))) {
        ld_next_token(p);
        ld_expect(p, '=');
...
static int ld_parse_section_pattern(LDParser *p, LDOutputSection *os,
                                    int keep) {
    ...
    while (p->tok != ')' && p->tok != LDTOK_EOF) {
      if (p->tok == LDTOK_NAME || p->tok == '.') {
        if (os->nb_patterns < LD_MAX_SECTION_PATTERNS) {
          ...
          pat->keep = keep;
          pat->type =
              (strchr(pat->pattern, '*') != NULL) ? LD_PAT_GLOB : LD_PAT_EXACT;
          os->nb_patterns++;
        }
        ld_next_token(p);
      } else {
  ...
  while (p->tok != '}' && p->tok != LDTOK_EOF) {
    if (p->tok == '.') {
      ...
      if (!strcmp(p->tok_buf, "KEEP")) {
        ...
        ld_expect(p, ')');
      } else if (!strcmp(p->tok_buf, "PROVIDE") ||
                 !strcmp(p->tok_buf, "PROVIDE_HIDDEN")) {
        ...
        if (p->tok == LDTOK_NAME) {
          int idx = ld_script_find_or_create_symbol(p->ld, p->tok_buf);
          if (idx >= 0) {
            p->ld->symbols[idx].visibility =
                hidden ? LD_SYM_PROVIDE_HIDDEN : LD_SYM_PROVIDE;
            ld_next_token(p);
            if (p->tok == '=') {
              ld_next_token(p);
  ...
  while (p->tok != '}' && p->tok != LDTOK_EOF) {
    if (p->tok == '.') {
      ...
      if (p->tok == '>') {
        ld_next_token(p);
        if (p->tok == LDTOK_NAME) {
          os->memory_region_idx =
              ld_script_find_memory_region(p->ld, p->tok_buf);
          ld_next_token(p);
        }
      }
...
int ld_script_parse(TCCState *s1, LDScript *ld, int fd) {
  ...
  while (parser.tok != LDTOK_EOF && ret == 0) {
    if (parser.tok == LDTOK_NAME) {
      if (!strcmp(parser.tok_buf, "MEMORY")) {
      ...
      } else if (!strcmp(parser.tok_buf, "OUTPUT_FORMAT") ||
                 !strcmp(parser.tok_buf, "OUTPUT_ARCH") ||
                 !strcmp(parser.tok_buf, "TARGET") ||
                 !strcmp(parser.tok_buf, "SEARCH_DIR") ||
                 !strcmp(parser.tok_buf, "INPUT") ||
                 !strcmp(parser.tok_buf, "GROUP") ||
                 !strcmp(parser.tok_buf, "OUTPUT") ||
                 !strcmp(parser.tok_buf, "INCLUDE")) {
        ...
        if (parser.tok == '=') {
          ...
          if (idx >= 0) {
            ld_next_token(&parser);
            ld->symbols[idx].value = ld_parse_expr(&parser);
...
void ld_script_dump(LDScript *ld) {
  ...
  for (int i = 0; i < ld->nb_memory_regions; i++) {
    ...
    printf("  %s (%c%c%c) : ORIGIN = 0x%lx, LENGTH = 0x%lx\n", mr->name,
           (mr->attributes & LD_MEM_READ) ? 'r' : '-',
           (mr->attributes & LD_MEM_WRITE) ? 'w' : '-',
           (mr->attributes & LD_MEM_EXEC) ? 'x' : '-',
  ...
  for (int i = 0; i < ld->nb_output_sections; i++) {
    ...
    for (int j = 0; j < os->nb_patterns; j++) {
      printf("    %s%s%s\n", os->patterns[j].keep ? "KEEP(" : "",
             os->patterns[j].pattern, os->patterns[j].keep ? ")" : "");
    }
    printf("  }");
