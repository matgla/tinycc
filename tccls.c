...
void tcc_ls_add_live_interval(LSLiveIntervalState *ls, int vreg, int start,
  ...
  if (ls->next_interval_index >= ls->intervals_size) {
    ...
    ls->active_set = (LSLiveInterval **)tcc_realloc(
        ls->active_set, sizeof(LSLiveInterval *) * ls->intervals_size);
  }
  ...
  printf(", start=%d end=%d crosses_call=%d addrtaken=%d reg_type=%d\n", start,
         end, crosses_call, addrtaken, reg_type);
  interval->end = end;
  interval->r0 = -1;
  interval->r1 = -1;
 ...
 * soft-float). Returns first register of pair, or -1 if no pair available. The
 * pair is (reg, reg+1), so we need to find an even register where both are
 * free. ARM EABI requires doubleword values in R0:R1 or R2:R3 for argument
 * passing, so we try even-aligned pairs first (R0:R1, R2:R3, R4:R5, etc.) */
int tcc_ls_assign_register_pair(LSLiveIntervalState *ls, int *r0_out,
                                int *r1_out) {
...
int tcc_ls_assign_callee_saved_register_pair(LSLiveIntervalState *ls,
                                             int *r0_out, int *r1_out) {
...
void tcc_ls_expire_old_intervals(LSLiveIntervalState *ls, int current_index) {
  ...
  for (int i = 0; i < ls->next_active_index; ++i) {
    ...
    if (ls->active_set[i]->reg_type == LS_REG_TYPE_FLOAT) {
      ...
      if (ls->active_set[i]->r1 >= 0 &&
          (ls->active_set[i]->reg_type == LS_REG_TYPE_LLONG ||
           ls->active_set[i]->reg_type == LS_REG_TYPE_DOUBLE_SOFT)) {
...
void tcc_ls_mark_float_register_as_used(LSLiveIntervalState *ls, int reg) {
  ...
  fprintf(stderr,
          "Error: trying to mark unallocatable float register %d as used\n",
          reg);
  exit(1);
}
...
int tcc_ls_next_stack_location() { return tcc_ls_next_stack_location_sized(4); }

/* Spill interval to stack. For doubles, allocates 8 bytes. */
void tcc_ls_spill_interval_sized(LSLiveIntervalState *ls, int interval_index,
                                 int size) {
...
void tcc_ls_allocate_registers(LSLiveIntervalState *ls,
  ...
  for (int i = 0; i < ls->next_interval_index; ++i) {
    ...
    if (ls->intervals[i].reg_type == LS_REG_TYPE_FLOAT ||
        ls->intervals[i].reg_type == LS_REG_TYPE_DOUBLE) {
      /* For VFP doubles, always spill to stack for now since the register
       * allocator doesn't properly handle D-register pairs (S0+S1, S2+S3, etc.)
       * and conversion operations use D0 as scratch */
      ...
      if (ls->intervals[i].r0 == -1) {
        ...
        /* r0 already contains the VFP register index - extract it, assign, and
         * re-add marker */
        int vfp_idx = LS_IS_VFP_REG(ls->intervals[i].r0)
                          ? LS_VFP_REG_NUM(ls->intervals[i].r0)
                          : ls->intervals[i].r0;
    ...
    } else if (ls->intervals[i].reg_type == LS_REG_TYPE_LLONG ||
               ls->intervals[i].reg_type == LS_REG_TYPE_DOUBLE_SOFT) {
      ...
      if (ls->intervals[i].r0 == -1) {
        ...
        if (ls->intervals[i].r0 >= 0) {
          ...
          if (preferred_r1 != 13 && preferred_r1 != 15) { /* Not SP or PC */
