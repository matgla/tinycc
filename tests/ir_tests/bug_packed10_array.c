...
static TestOperand pool_read(TestOperand *pool, int base, int off)
{
 ...
 * then read bitfield. */
__attribute__((noinline))
static int get_tag(TestOperand *pool, int base, int off)
{
...
__attribute__((noinline))
static int32_t get_imm32(TestOperand *pool, int base, int off)
{
    TestOperand op = pool[base + off];
    return op.u.imm32;
}
...
#define ENCODE_PARAM(call_id, param_idx) \
    ((int32_t)(((uint32_t)(call_id) << 16) | ((uint32_t)(param_idx) & 0xFFFFu)))
