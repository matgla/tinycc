...
def build_make_command(test_file, machine, compiler):
    ...
    return f'make -C {make_dir} OUTPUT={CURRENT_DIR}/build TEST_FILES={test_file} CC={compiler} TARGET={get_test_output_file(test_file)}'
...
def compile_testcase(test_file, machine, compiler=f"{CURRENT_DIR}/../../armv8m-tcc"):
    ...
    if not was_cleaned:
        result = subprocess.run(make_command + " clean", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
