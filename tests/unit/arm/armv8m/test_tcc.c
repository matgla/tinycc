...
UT_SUITE(tcc)
{
  ...
  UT_RUN(test_main_preprocess_without_outfile_uses_stdout);
  ...
  UT_RUN(test_main_just_deps_skips_output_file);
  ...
  UT_RUN(test_main_opt_m64_returns_one);

  UT_RUN(test_main_default_output_type_to_exe);
  UT_RUN(test_main_output_file_without_outfile_uses_default);
  UT_RUN(test_main_preprocess_outfile_fopen_failure);
  UT_RUN(test_main_add_file_error_returns_one);
  UT_RUN(test_main_missing_end_group_proper);
  UT_RUN(test_main_unmatched_end_group_proper);
  UT_RUN(test_main_nested_group);
  UT_RUN(test_main_verbose_group_prints_file);
  UT_RUN(test_main_group_satisfiable_undefs_false);
  UT_RUN(test_main_group_rescan_breaks_when_nothing_loaded);
  UT_RUN(test_main_group_rescan_processes_archive_file);
  ...
  UT_RUN(test_is_64bit_operand_pointer_is_false);
  UT_RUN(test_is_64bit_operand_void_is_false);
  ...
  UT_RUN(test_default_outputfile_dll_falls_back_to_a_out);
}
